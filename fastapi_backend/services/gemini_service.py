...
def parse_pdf_to_text(file_path: str) -> str:
    ...
    try:
        ...
        print(f"[Gemini Service] Lỗi đọc PDF bằng fitz, thử lại bằng pdfplumber: {e}")
        try:
            ...
            print(f"[Gemini Service] Lỗi đọc PDF sang Text: {e2}")
    return text
...
def find_matching_department_id(dept_keyword: str, departments: list) -> str:
    ...
    # 0. So khớp trực tiếp với ID của phòng ban (Ví dụ: "nv7" == "nv7")
    for dept in departments:
        d_id = dept["id"].lower().strip()
        if clean_kw == d_id:
            return dept["id"]
    ...
    # Mới: Sử dụng từ điển viết tắt từ user.md để dịch nghĩa
    try:
            ...
            with open(user_md_path, "r", encoding="utf-8") as f:
                for line in f:
                    if ":" in line:
                        ...
                        if clean_kw == k:
                            # So khớp nghĩa đã dịch trong tên phòng ban
                            for dept in departments:
                                dept_name = dept["name"].lower()
                                if v in dept_name or dept_name in v:
                                    return dept["id"]
    except Exception as e:
        print("Lỗi đọc user.md trong find_matching_department_id:", e)
...
def remove_vietnamese_accents(s: str) -> str:
    """Loại bỏ dấu tiếng Việt để phục vụ so khớp chuỗi không dấu."""
    s = s.lower()
...
def match_participant_to_user(raw_name: str, users: list, departments: list) -> str | None:
    ...
    for u in filtered_users:
        ...
        if len(name_accentless.split()) > 1:
            if name_accentless in u_username or u_username in name_accentless:
                return u["id"]
        else:
